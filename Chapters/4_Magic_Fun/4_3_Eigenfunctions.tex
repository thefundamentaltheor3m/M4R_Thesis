...
As a preliminary step, though, we need to show integrability. % We only sketch the proof.
    Effectively, we need to show that % the function
    \begin{align*}
        (x, z) \mapsto f\of{\norm{x}^2, z}
    \end{align*}
    ...
    \end{enumerate}
    These easily follow from the arguments in \Cref{Ch4:Subec:Schwartzness_a}, and we can therefore freely swap integrals using Fubini's theorem to prove the eigenfunction property.
...
\end{boxlemma}
\begin{proof}
    Since $\F$ acts linearly, we can treat $I_1$ and $I_2$ separately. For the purpose of this proof, denote the Fourier transforms of $I_1\of{\norm{x}^2}$ and $I_2\of{\norm{x}^2}$ by $F_1$ and $F_2$ respectively. Fix $\xi \in \R^8$. \Cref{Ch4:Prop:FourierIntegralConvergence_Ij} allows us to write
    \begin{align*}
        F_1\of{\xi}
        % &= \int_{\R^8} \parenth {\int_{-1}^{-1 + i} \phi_0\of{\frac{-1}{z+1}} \, \parenth{z + 1}^2 \, e^{\pi i \norm{x}^2 z} \, \diff{z}} e^{-2\pi i \cycl{x, \xi}} \, \diff{x} \\
    ...
    %     \frac{1}{z^4} \, e^{-\pi i \norm{\xi}^2 / z}
    % \end{align*}
    We may therefore apply \Cref{Ch4:Thm:GaussianFourier} and write
    \begin{align*}
    ...
    \end{align*}
    by \eqref{Ch4:Eq:phi_0_add_one}. One can show $f$ to be holomorphic (in $w$) on $\Halfplane$ and continuous on the closed regions in question because of \eqref{Ch4:Eq:phi_0_neg_inv}. Applying \Cref{Ch4:Thm:CGTRectCircle}, followed by an analogous, \href{https://github.com/leanprover-community/mathlib4/blob/88928cefd7edb1ba61623bffd4e86389dfe1f648/Mathlib/Analysis/Complex/CauchyIntegral.lean#L245}{previously formalised} result, we can show that changing contours as we do in \Cref{Ch4:Fig:Eigenfunction_CauchyGoursat_a} then tells us that
    % \begin{align*}
    %     F_1\of{\xi}
    %     &= \int_{\gamma_1} \fof{w, \xi} \, \diff{w}
    %     = \int_{1}^{1 + \frac{1}{2}i} \fof{w, \xi} \, \diff{w}
    %     + \int_{1 + \frac{i}{2}i}^{\frac{1}{2} + \frac{1}{2}i} \fof{w, \xi} \, \diff{w} \\
    %     F_2\of{\xi}
    %     &= \int_{\gamma_2} \fof{w, \xi} \, \diff{w}
    %     = \int_{\frac{1}{2} + \frac{1}{2}i}^{\frac{1}{2} + i} \fof{w, \xi} \, \diff{w}
    %     + \int_{\frac{1}{2} + i}^{i} \fof{w, \xi} \, \diff{w}
    % \end{align*}
    % The \CGT\ for rectangles, which has been , tells us that
    % \begin{align*}
    %     \int_{1 + \frac{i}{2}i}^{\frac{1}{2} + \frac{1}{2}i} \fof{w, \xi} \, \diff{w}
    %     + \int_{\frac{1}{2} + \frac{1}{2}i}^{\frac{1}{2} + i} \fof{w, \xi} \, \diff{w}
    %     = \int_{1 + \frac{1}{2}i}^{1+i} \fof{w, \xi} \, \diff{w}
    %     + \int_{1+i}^{\frac{1}{2} + i} \fof{w, \xi} \, \diff{w}
    % \end{align*}
    % Therefore,
    \begin{align*}
        F_1\of{\xi} + F_2\of{\xi}
        % &= \int_{1}^{1 + \frac{1}{2}i} \fof{w, \xi} \, \diff{w}
        % + \parenth{\int_{1 + \frac{i}{2}i}^{\frac{1}{2} + \frac{1}{2}i} \fof{w, \xi} \, \diff{w}
        % + \int_{\frac{1}{2} + \frac{1}{2}i}^{\frac{1}{2} + i} \fof{w, \xi} \, \diff{w}}
        % + \int_{\frac{1}{2} + i}^{i} \fof{w, \xi} \, \diff{w} \\
        % &= \parenth{\int_{1}^{1 + \frac{1}{2}i} \fof{w, \xi} \, \diff{w}
        % + \int_{1 + \frac{1}{2}i}^{1+i} \fof{w, \xi} \, \diff{w}}
        % + \parenth{\int_{1+i}^{\frac{1}{2} + i} \fof{w, \xi} \, \diff{w}
        % + \int_{\frac{1}{2} + i}^{i} \fof{w, \xi} \, \diff{w}} \\
        &= I_3\of{\norm{\xi}^2} + I_4\of{\norm{\xi}^2}
    \end{align*}
...
The proof that the Fourier transform maps $I_5$ to $I_6$ is nearly identical in structure but significantly simpler, because the Möbius transformation $z \mapsto \frac{-1}{z}$ simply maps the contour in $I_5$ to that in $I_6$ (and vice-versa). In summary, the Fourier transform permutes the integrals that make up $a$, thereby not changing $a$. $a$ is thus an eigenfunction of $\F$ with eigenvalue $1$.
...
First, we note that we have integrability for the same reason we did in \Cref{Ch4:Subsec:Eigenfun_a}. We thus prove an analogue of \Cref{Ch4:Lemma:Fourier_I_1_add_I_2_eq_I_3_add_I_4}.
...
\begin{proof}
    ...
    \begin{align*}
        ...
        = -\psi_T\of{w} \, e^{\pi i \norm{\xi}^2 w}
    \end{align*}
    by \eqref{Ch4:Eq:psi_T_eq_zsq_mul_psi_T_neg_inv}. Noting further that $f$ is holomorphic in $w$ because of \Cref{Ch4:Prop:psi_as_div_disc}, we can prove the result by deforming contours in exactly the same manner as we did in the proof of \Cref{Ch4:Lemma:Fourier_I_1_add_I_2_eq_I_3_add_I_4}. See \Cref{Ch4:Fig:Eigenfunction_CauchyGoursat_a}.
\end{proof}
