...
% \end{proof}
Finally, we can apply \Cref{Ch2:Lemma:Slash_Mul} to show that
\begin{align*}
    \psi_T \mid_{-2} S = \psi_I \mid_{-2} \parenth{TS}
\end{align*}
...
Note that by applying \eqref{Ch4:Eq:psi_T_to_psi_S_add} and \eqref{Ch4:Eq:psi_I_to_psi_S}, we can express $J_1, \ldots, J_5$ in the following manner:
\begin{align*}
    ...
    J_5(r) &= -2 \int_{0}^{i} \psi_S\of{\frac{-1}{z}} \, z^2 \, e^{\pi i n z} \, \diff{z}
\end{align*}
With $J_1, \ldots, J_5$ expressed in this manner, and $J_6$ expressed as in \eqref{Ch4:Eq:J_6_def}, there is a marked visual similarity between the $J_j$ and the $I_j$, with $\psi_S$ being the analogue of $\phi_0$. A consequence of this is that similar strategies can be used to prove properties about both $a$ and $b$.