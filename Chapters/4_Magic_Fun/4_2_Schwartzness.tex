...
\begin{proof}
    ...
    \end{align*}
    we have, for $\ell \geq n_0$,
    \begin{align*}
        c(\ell) = \sum_{n = n_1}^{\ell - n_2} c_1(n) c_2(\ell - n)
    \end{align*}
    For large $\ell$, both $n$ and $\ell-n$ are large for most terms in the sum, so
    \[
        |c_1(n)| \leq C n^{k_1}, \quad |c_2(\ell-n)| \leq D (\ell-n)^{k_2}
    \]
    Thus,
    \begin{align*}
        |c(\ell)| &\leq \sum_{n = n_1}^{\ell - n_2} |c_1(n)|\,|c_2(\ell-n)| \\
        &\leq \sum_{n = n_1}^{\ell - n_2} C n^{k_1} D (\ell-n)^{k_2} \\
        &\leq CD \sum_{n=0}^{\ell} n^{k_1} (\ell-n)^{k_2}
    \end{align*}
    The sum is bounded above by $CD \cdot \ell \cdot \ell^{k_1} \ell^{k_2} = CD \cdot \ell^{k_1 + k_2 + 1}$, so $|c(\ell)| = O(\ell^{k_1 + k_2 + 1})$ as $\ell \to \infty$. The lowest index with nonzero coefficient is $n_0 = n_1 + n_2$, since $c_1(n_1) \neq 0$ and $c_2(n_2) \neq 0$, so $c(n_0) = c_1(n_1)c_2(n_2) \neq 0$. This completes the proof.
\end{proof}
...
A theme we will see repeatedly is that there are analogues for the $-1$-eigenfunction of many results on the $+1$-eigenfunction. We will aim to avoid repetition by only discussing in full detail those results that apply exclusively to the $-1$-eigenfunction.

The main step in which the proof that $b\rad$ is Schwartz differs from the proof that $a\rad$ is Schwartz is the step in which we show that \Cref{SP:PolyFourierCoeffBound} is applicable.