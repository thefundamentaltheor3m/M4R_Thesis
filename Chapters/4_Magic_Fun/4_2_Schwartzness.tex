...
\end{boxlemma}
\begin{proof}
    From \eqref{Ch2:Eq:H2_qexpansion_explicit}, we can see that $n_0\of{H_2} = 1$. Furthermore, from \Cref{Ch2:Def:Theta_H}, we can see that $n_0\of{\Theta_3} = n_0\of{\Theta_4} = 0$, from which we can conclude that $n_0\of{H_3} = n_0\of{H_4} = 0$. Finally, $c_{H_2}\of{1} = 16$ from \eqref{Ch2:Eq:H2_qexpansion_explicit} and $c_{H_2} = c_{H_3} = 1$ from \Cref{Ch2:Def:Theta_H}. Now, denote by $N_I$, $N_S$ and $N_T$ the values of $n_0$ for the numerators (cf. \Cref{Ch4:Prop:psi_as_div_disc}) of $\psi_I$, $\psi_S$ and $\psi_T$ respectively.
    \begin{itemize}
