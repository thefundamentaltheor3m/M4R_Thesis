...
It is clear that the estimates for $\abs{I_2}$, $\abs{I_4}$ and $\abs{I_6}$ have at least exponential decay, making them more rapidly decaying than any inverse power of $r$. For $\abs{I_1}$, $\abs{I_3}$ and $\abs{I_5}$, the result is actually a consequence of deeper results involving the Gamma function.
...
\begin{proof}
    ...
    \end{align*}
    for all $x > 0$. Hence, writing $u = 2\pi s$ and relating $\Gamma$ with the factorial, which was also \href{https://github.com/leanprover-community/mathlib4/blob/5a2eaa85c555c4263e15928cef249cbaad2eb2d2/Mathlib/Analysis/SpecialFunctions/Gamma/Basic.lean#L430}{previously formalised} we get
    \begin{align*}
...
An important comment we make here is that the fact that the functions that the bounding functions decay also shows that the integrals $I_j$ converge absolutely.

Next, we show that the $I_j$ are smooth and that their derivatives satisfy similar bounds to the ones computed above. The key to proving this is the Leibniz Integral Rule, which was also \href{https://github.com/leanprover-community/mathlib4/blob/dd2606726d6b0601d394911f9e7a6fa0f01367a3/Mathlib/Analysis/Calculus/ParametricIntegral.lean#L281-L303}{formalised previously}.
...
\begin{comment} % This is unnecessary because we have the psi_T |-2 TSTS relation
The integrals for which this is not true purely by inspection are $J_2$ and $J_4$, because the integrands take a slightly different form. However, the difference is only inside $\psi_S$, and we can show that this does not affect the imaginary part of the input particularly profoundly.

\begin{boxlemma}\label{Ch4:Lemma:Bound_J3}
    There exists a positive real number $C_S$ such that for all $r \in \R$,
    \begin{align*}
        \abs{J_3(r)} &\leq \int_{1}^{\infty} C_T \, e^{-\pi s} \, e^{-\pi r/s} \, \diff{s}
    \end{align*}
\end{boxlemma}
\begin{proof}
    Parametrise $z = 1 + it$ in \eqref{Ch4:Eq:J3_like_I3}. Then, for all $r \in \R$, we can write
    \begin{align*}
        J_3(r) &= i \int_{0}^{1} \psi_S\of{\frac{-1 -it}{it}} \, t^2 \, e^{\pi i r} \, e^{-\pi r t} \, \diff{z}
    \end{align*}
    Writing $s = \frac{1}{t}$ and simplifying, we get that
    \begin{align*}
        J_3(r) &= i \int_{1}^{\infty} \psi_S\of{si - 1} \, s^{-4} \, e^{\pi i r} \, e^{-\pi r / s} \, \diff{z}
    \end{align*}
    Observe that $\Im\of{si - 1} = s > \frac{1}{2}$ inside the integral. We can therefore bound $\abs{\psi_S\of{si - 1}}$ using \eqref{Ch4:Eq:PolyFourierCoeffBound_psi_S} and bound $\abs{J_3}$ analogously to how we bounded $\abs{I_1}$ in \Cref{Ch4:Lemma:Bound_I1_I3_I5}.
\end{proof}

In similar fashion, we can bound $J_4$ by an exponentially decaying function.
...
Finally, we note that the process of reducing the higher-derivative case to the zeroth-derivative case is, once again, analogous to \Cref{Ch4:Lemma:Reducing_higher_deriv_Schwartzness_I_1}: the key idea of the proof is to express the $k$th derivative as a constant (to the $k$th power) times the $0$th derivative, and this does not depend on whether the integrand contains $\phi_0$ or $\psi_S$. We end by noting that the functions bounding $\abs{J_2}$, $\abs{J_4}$ and $\abs{J_6}$ are obviously Schwartz, and those bounding $\abs{J_1}$, $\abs{J_3}$ and $\abs{J_5}$ are Schwartz by an argument almost verbatim to \Cref{Ch4:Lemma:Bessel_Bound}.
