...
It is worth mentioning that these tactics do not always simplify expressions in $\R$, either. This is due to the immense technical detail baked into the very definition of $\R$. That being said, expressions in $\R$ that are images of expressions in $\Q$ or $\Z$ via the canonical inclusions\footnote{We would call these `coercions' or `casts' in Lean} are often handled by \lstinline|norm_num|. In $\C$, there are numerous 
