...
Somewhat unexpectedly, the $\sigma$-function shows up in the Fourier expansions of the Eisenstein series. The fact that $\sigma$ is well-behaved and that its behaviour is easily formalised makes it easy to prove properties about the Fourier coefficients of the Eisenstein series, which we will need to do for Schwartzness proofs later on.

We now give the Fourier expansions of the Eisenstein Series.
...
We finally turn our attention to $E_2$. It is tempting to define $E_2$ via the summation used in \Cref{Ch2:Eq:Eisenstein_def_Chris} or \Cref{Ch2:Eq:Eisenstein_def_with_zeta_normalisation}. Unfortunately, when $k = 2$, these sums do not converge absolutely. That being said, Birkbeck has \href{https://github.com/thefundamentaltheor3m/Sphere-Packing-Lean/blob/076f4b8d6a37fa95de3bc4764a5d7f911fde91e0/SpherePacking/ModularForms/summable_lems.lean#L1680}{shown formally} that for all $m \in \Z$, $z \in \Halfplane$, and $k \geq 2$, the summation
\begin{align*}
...
We treat this separately (ie, we do not include this fact in the statement of \Cref{Ch2:Thm:Ek_qexpansion}) because Birkbeck treats it separately in the repository. He gives a formal proof over the course of several \sorry-free lemmas in \href{https://github.com/thefundamentaltheor3m/Sphere-Packing-Lean/blob/076f4b8d6a37fa95de3bc4764a5d7f911fde91e0/SpherePacking/ModularForms/E2.lean#L736}{\texttt{SpherePacking.ModularForms.E2.lean}}. Interestingly, substituting $k = 2$ in \eqref{Ch2:Eq:Ck_Ek_qexpansion_const} yields precisely $-24$.

It is noteworthy that $E_2$ is \textit{not a weight $2$, level $1$ modular form}, in contrast to the fact that for $k \geq 4$ even, $E_k$ is a weight $k$, level $1$ modular form. The reason for this is that it does not quite exhibit weight $2$, level $1$ slash-invariance.
...
\subsection{The Jacobi Theta Functions}